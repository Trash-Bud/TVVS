...
class BoardFactoryTest {
    ...
    @Test
    void createBoardWithDifferentInputs() {
        assertThat(factory.createBoard(new Square[][]{{s1, s2}}).invariant()).isTrue().isNotNull();

        AssertionError error = Assertions.assertThrows(AssertionError.class, () -> {
            factory.createBoard(new Square[][]{{null,s1}});
        });
        Assertions.assertEquals("Initial grid cannot contain null squares", error.getMessage());

        error = Assertions.assertThrows(AssertionError.class, () -> {
            factory.createBoard(null);
        });

        assertThat(factory.createBoard(new Square[][]{{}}).invariant()).isTrue().isNotNull();
